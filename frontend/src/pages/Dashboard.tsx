...
const Dashboard = () => {
  ...
  const formatValue = (val: number) => {
    ...
    if (val >= 1000) return `${isCurrency ? '₹' : ''}${(val/1000).toFixed(1)}K`;
    return `${isCurrency ? '₹' : ''}${val.toLocaleString()}`;
  };
    ...
    <div className="space-y-6 pb-10 max-w-full">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-brand-muted text-sm mt-1">AI-Powered Planning (Target: <strong className="text-brand-accent">{metricName}</strong>)</p>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between items-start sm:items-center gap-4 mb-6">
          ...
          <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
            ...
            <button onClick={handleGenerateForecast} disabled={loading} className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-brand-accent hover:bg-blue-600 text-white px-4 py-2.5 rounded-lg text-sm font-medium transition-all">
              {loading ? <Loader2 size={16} className="animate-spin" /> : <Sparkles size={16} />} Predict
            </button>
          </div>
        </div>
          ...
          {forecastData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={forecastData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                ...
                <YAxis stroke="#94a3b8" tick={{fontSize: 11}} axisLine={false} tickLine={false} tickFormatter={formatValue} />
                <Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#1e293b', color: '#f8fafc' }} formatter={(val: any) => [`${isCurrency ? '₹' : ''}${Number(val).toLocaleString()}`, metricName]} />
                <Area type="monotone" dataKey="predicted_revenue" stroke="#3b82f6" strokeWidth={3} fillOpacity={1} fill="url(#colorRev)" />
              </AreaChart>
            </ResponsiveContainer>
          ) : (
            <div className="h-full flex items-center justify-center text-brand-muted border border-dashed border-slate-700 rounded-lg text-center px-4">Upload data or click Predict to view AI forecast</div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-brand-card p-4 md:p-6 rounded-xl border border-slate-800 overflow-hidden w-full flex flex-col">
          ...
          <div className="h-[250px] w-full min-w-0">
            {scenarioData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={scenarioData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" vertical={false} />
                  <XAxis dataKey="date" stroke="#94a3b8" tick={{fontSize: 11}} tickFormatter={(v) => v.substring(5)} axisLine={false} tickLine={false} />
                  <YAxis stroke="#94a3b8" tick={{fontSize: 11}} axisLine={false} tickLine={false} tickFormatter={formatValue} />
                  <Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#1e293b' }} formatter={(val: any) => [`${isCurrency ? '₹' : ''}${Number(val).toLocaleString()}`]} />
        ...
        {/* ANOMALIES */}
        <div className="bg-brand-card p-4 md:p-6 rounded-xl border border-slate-800 overflow-hidden w-full">
          <div className="flex items-center gap-2 mb-6"><AlertTriangle size={20} className="text-orange-400 shrink-0" /> <h2 className="text-lg font-bold text-white">AI Outlier Alerts</h2></div>
            ...
            {anomalies.length > 0 ? anomalies.map((anomaly, idx) => (
              <div key={idx} className="p-3 border border-slate-700 bg-slate-800/30 rounded-lg flex flex-col gap-2 hover:bg-slate-800/80 transition-colors">
                <div className="flex justify-between items-start">
                  <span className={`text-xs font-bold px-2 py-1 rounded-md ${anomaly.severity.includes('Critical') ? 'bg-red-500/20 text-red-400' : anomaly.severity.includes('Major') ? 'bg-orange-500/20 text-orange-400' : 'bg-blue-500/20 text-blue-400'}`}>{anomaly.severity}</span>
                ...
                <div className="flex flex-wrap justify-between items-center mt-1 gap-2">
                  ...
                  <span className="text-sm font-bold text-slate-300">{isCurrency ? '₹' : ''}{Number(anomaly.revenue).toLocaleString()}</span>
                </div>
              </div>
            )) : (
              <div className="text-center text-brand-muted text-sm mt-10">No recent anomalies detected in data.</div>
            )}
          </div>
        </div>
