...
const Dashboard = () => {
  ...
  const fetchAnomalies = async () => {
    try {
      ...
      setAnomalies(res.data.slice(0, 5)); // Show top 5 recent anomalies
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <div className="flex justify-between items-center">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div className={`flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-medium border ${serverStatus === 'online' ? 'bg-green-500/10 text-green-400 border-green-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'}`}>
      ...
      <div className="bg-brand-card p-6 rounded-xl border border-slate-800">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-3">
            <select value={horizon} onChange={(e) => setHorizon(Number(e.target.value))} className="bg-slate-800 border border-slate-700 text-white text-sm rounded-lg p-2.5 outline-none">
              <option value={7}>7 Days</option>
              <option value={30}>30 Days</option>
              <option value={90}>90 Days</option>
            </select>
            <button onClick={handleGenerateForecast} disabled={loading} className="flex items-center gap-2 bg-brand-accent hover:bg-blue-600 text-white px-4 py-2.5 rounded-lg text-sm font-medium transition-all">
          ...
          {forecastData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={forecastData}>
                ...
                <XAxis dataKey="date" stroke="#94a3b8" tick={{fontSize: 12}} tickFormatter={(v) => v.substring(5)} axisLine={false} tickLine={false} />
                <YAxis stroke="#94a3b8" tick={{fontSize: 12}} axisLine={false} tickLine={false} tickFormatter={(v) => `₹${(v/1000000).toFixed(1)}M`} />
            ...
            <div className="h-full flex items-center justify-center text-brand-muted border border-dashed border-slate-700 rounded-lg">Click Generate to view baseline forecast</div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* 3. WHAT-IF SIMULATOR (Takes up 2 columns) */}
        <div className="lg:col-span-2 bg-brand-card p-6 rounded-xl border border-slate-800">
          <div className="flex items-center gap-2 mb-6">
            <Sliders size={20} className="text-purple-400" />
            <h2 className="text-lg font-bold text-white">What-If Scenario Simulator</h2>
          </div>
          
          <div className="flex flex-col md:flex-row gap-6 mb-6 p-4 bg-slate-800/50 rounded-lg border border-slate-700">
            ...
            <button onClick={handleSimulate} disabled={simLoading} className="bg-purple-600 hover:bg-purple-500 text-white px-6 py-2 rounded-lg font-medium self-end transition-all">
            ...
            {scenarioData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={scenarioData}>
                  ...
                  <Line type="monotone" dataKey="simulated_revenue" name="Simulated Scenario" stroke="#a855f7" strokeWidth={3} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            ) : (
              <div className="h-full flex items-center justify-center text-brand-muted">Adjust sliders and run simulation to compare scenarios.</div>
            )}
          </div>
          
          {scenarioSummary && (
            <div className="mt-4 p-3 bg-slate-800 rounded-lg flex justify-between items-center text-sm">
        ...
        {/* 4. AI ANOMALY ALERTS (Takes up 1 column) */}
        <div className="bg-brand-card p-6 rounded-xl border border-slate-800">
          <div className="flex items-center gap-2 mb-6">
